...
class FingersNumberDetector:
    ...
    def __init__(self, cap):
        ...
        self.setupFrame(self.frame_width, self.frame_height)
        self.cnt = 0
    ...
    def detectHand(self, frame, handHist):
        ...
        if len(contours) > 0:
            ...
            self.countFingers(maxContour, contourAndHull)
...
if __name__ == "__main__":
    ...
    detector = FingersNumberDetector(cap)
    ...
    while cap.isOpened():
        ...
        # Gamma correction
        # Increase the contrast
        # frame = cv2.LUT(frame, detector.lookUpTable)

        cv2.rectangle(frame, (detector.x0, detector.y0),
                      (detector.x0 + detector.width - 1, detector.y0 + detector.height - 1), (255, 0, 0), 2)

        k = cv2.waitKey(1) & 0xFF

        if k == ord("z"):
            detector.isHandHistCreated = True
            handHist = detector.createHandHistogram(frame)
        elif k == ord('b'):
            detector.bgSubtractor = cv2.createBackgroundSubtractorMOG2(10, detector.bgSubThreshold)
            detector.isBgCaptured = True
        elif k == ord("r"):
            detector.bgSubtractor = None
            detector.isBgCaptured = False

        if detector.isHandHistCreated and detector.isBgCaptured:
            detector.detectHand(frame, handHist)
        elif not detector.isHandHistCreated:
        ...
        if k == ord("q"):
            ...
            detector.y0 = min(detector.y0 + 20, detector.frame_height - detector.height)
        elif k == ord("k"):
            detector.y0 = max(detector.y0 - 20, 0)
        elif k == ord("h"):
            detector.x0 = max(detector.x0 - 20, 0)
        elif k == ord("l"):
            detector.x0 = min(detector.x0 + 20, detector.frame_width - detector.width)
